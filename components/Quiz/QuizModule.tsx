    ...
    return (
      <div className="max-w-2xl mx-auto py-12">
        <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-12 text-center shadow-2xl">
          ...
          <h2 className="text-4xl font-black mb-4 tracking-tight">{t.quiz.completedTitle}</h2>
          <p className="text-slate-400 text-lg mb-10">{t.quiz.completedDesc}</p>

          <div className="grid grid-cols-2 gap-6 mb-12">
            <div className="bg-slate-800/50 p-6 rounded-3xl border border-slate-700">
              <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t.quiz.finalScore}</div>
              <div className="text-4xl font-black text-indigo-400">{score} / {questions.length}</div>
            </div>
            <div className="bg-slate-800/50 p-6 rounded-3xl border border-slate-700">
              <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t.quiz.accuracy}</div>
    ...
    <div className="max-w-3xl mx-auto py-8 px-4">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="space-y-1">
          <div className="text-xs font-bold text-indigo-400 uppercase tracking-widest">{t.quiz.category}: {t.categories[currentQuestion.category === 'Dynamic Programming' ? 'dynamicProgramming' : currentQuestion.category.toLowerCase() as keyof typeof t.categories]}</div>
          <h2 className="text-2xl font-bold">{t.quiz.questionProgress.replace('{0}', (currentIdx + 1).toString()).replace('{1}', questions.length.toString())}</h2>
        </div>
        ...
        {!showFeedback ? (
          ...
          <div className="space-y-6">
            <div className="bg-slate-800/80 p-6 rounded-2xl border border-slate-700 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h4 className="font-bold text-slate-300 mb-2 uppercase tracking-widest text-xs">{t.quiz.explanation}</h4>
